...
(function () {
  ...
  document.getElementById('main').innerHTML = `
    <section class="pan cover" id="s00">
      <div class="dw">${pic(D.hero, 'hero')}<span class="capn" style="width:var(--tw,auto)">${esc(D.hero.cap)}</span></div>
      <div class="g-title">
    ...
    ${hasBefore
      ? `<div class="g-dwell" data-dwell data-wipe><section class="pan" id="s07">${TXT('07 BEFORE &amp; AFTER', D.s07.h || '요청이 한 곳으로 모였습니다', P(D.s07.p))}<div class="dw"><div class="stack">${pic(D.s07.before)}${pic(D.s07.after, 'after')}<i class="edge"></i></div><div class="wcaps"><span class="wb">${esc(D.s07.before.cap)}</span><span class="arr">→</span><span class="wa" style="opacity:.35">${esc(D.s07.after.cap)}</span></div></div></section></div>`
      : `<section class="pan" id="s07">${TXT('07 BEFORE &amp; AFTER', D.s07.h || '요청이 한 곳으로 모였습니다', P(D.s07.p))}<div class="dw">${pic(D.s07.after)}<span class="capn">${esc(D.s07.after.cap)}</span></div></section>`}
    <section class="pan finale" id="s09">
      <div class="two-col"><div class="txt"><span class="nlab">09 WHAT I LEARNED</span><h2>${D.s09.h || '할 일 앱을 하나 더<br>만들 뻔했습니다'}</h2>${P(D.s09.p)}</div>
  ...
  document.querySelectorAll('[data-morph]').forEach(d => {
    ...
    d._morph = t => { if (t === last) return; last = t; src.forEach((y, i) => { const b = B[i], a = A[i]; const v = b.map((n, j) => lerp(n, a[j], t)); const el = d.querySelector(`path[data-i="${i}"]`); el.setAttribute('d', `M40 ${y} C ${v[0]} ${v[1]}, ${v[2]} ${v[3]}, ${v[4]} ${v[5]}`); el.setAttribute('class', t > .5 ? 'after' : 'before'); }); d.querySelector('.mb').style.opacity = Math.max(0, 1 - t * 2); d.querySelector('.ma').style.opacity = clamp((t - .45) * 2, 0, 1); mchain.style.strokeDashoffset = L * (1 - clamp((t - .5) * 2, 0, 1)); d.querySelector('.all').setAttribute('transform', `translate(${(lerp(50, 22, t)).toFixed(1)},0)`); };   // 그림이 상자 가운데에 오게 상태별로 옮김
