...
window.V5 = (function () {
  ...
  V.init = function () {
    ...
    V.step = dir => { const at = lenis ? lenis.targetScroll : sl(); const t = dir > 0 ? snaps.find(v => v > at + 2) : [...snaps].reverse().find(v => v < at - 2); if (t === undefined) return false; snapping = true; moving = false; gestureFrom = null; if (lenis) lenis.scrollTo(t, { duration: 1.5, onComplete: () => { snapping = false; } }); else wrap.scrollTo({ left: t, behavior: 'smooth' }); return true; };
    ...
    wrap.addEventListener('wheel', e => {
      ...
      if (V.step(dir)) lockUntil = now + 700;              // 이동 중 추가 휠 무시(연타 방지)
    }, { passive: false });
