...
window.V5 = (function () {
  ...
  V.init = function () {
    ...
    // wheel / trackpad (A): 노치 하나 = 점 하나. 묶음 창(220ms) 안의 추가 노치는 버리지 않고 큐에 쌓아 순서대로 진행
    let acc = 0, lockUntil = 0, pending = 0;
    wrap.addEventListener('wheel', e => {
      ...
      if (now < lockUntil) { pending += dir; return; }
      V.step(dir); lockUntil = now + 220;
    }, { passive: false });
    gsap.ticker.add(() => { if (pending && performance.now() >= lockUntil) { const dir = Math.sign(pending); pending -= dir; V.step(dir); lockUntil = performance.now() + 220; } });
