...
| C4 | 장 폭: 표지 100vw, 01·02·09 44rem, 03~05 묶음 100vw, 06 80rem, 07 80rem, 08 64rem, 10 100vw. **바닥선 문법**: 모든 내용은 선 위에 선다(밑변 = 선 − 28px), 캡션만 선 아래 12px. 헤더 아래 ~ 선 위 8px 사이를 벗어나는 요소 없음 | 자동: 각 장의 내용 rect가 [72, ly−8] 안 |
| C5 | 선(S1): 트랙 폭 SVG, y = 62vh 고정. 점 = 장 시작 + 48px. 잉크 끝 = x + 45vw. 지난 점 채움 | 자동: `line.y === innerHeight*.62 ± 1`, 점 11개 |
| C6 | 라이브 화면 배율 = min(폭/1280, 높이/800). 높이 = 장 위 여백~선 사이(그림 칸) 또는 거기서 제목·문단을 뺀 나머지(07). 1280×720에서도 잘리지 않음 | 자동: `.shot.live` rect가 장 rect 안 |
...
| C4 | 통과 | 바닥선 배치로 재검증: 1280×720·1440×900·1920×1080 모두 헤더~선 사이 위반 0. 1280×720은 묶음 장 세 칸·표지 세 칸 규칙 |
| C5 | 통과 | ly = 62vh(558@900, 446@720, 670@1080), 점 9 |
| C6 | 통과 | 배율 720: .29~.41, 900: .40~.53, 1080: .54~.67. 화면 rect가 장 안 |
...
### 9.1 배치 수정 (2026-09-21, 사용자 지적: 선·그림·글 겹침)
- 선 SVG가 `.track`에 있어 장과 함께 움직이지 않았다(점이 항상 x=48에 보임) → `.scenes` 안 첫 자식으로 옮김.
- 장 내용이 위에서 시작해 아래 절반이 비고 선이 그림을 관통했다 → **바닥선 문법**(docs/18): 장 아래 여백 = 38vh + 28, 내용은 선 위에 서고 캡션은 선 아래.
- v3.css의 `.step/.steps/.num/.nums` 규칙과 충돌(번호에 취소선, 묶음 위 괘선) → `.gstep/.knums`로 이름 변경.
- 표지 왼쪽 여백을 헤더 브랜드 시작선(112)에 맞춤. 07 캡션 겹침(absolute 컨테이너 폭 0) → 폭을 화면 폭으로. 와이프 경계선의 둥근 손잡이 제거(선 위 점과 혼동).
